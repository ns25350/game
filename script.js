...
// ★初期ステータスに「強化大成功時の倍率 (upgradeCritMult: 2)」を追加
let playerStats = { maxHp: 1000, baseAtk: 10, critRate: 5, critDmg: 50, upgradeCritMult: 2 };
...
const checkRulesBtn = document.getElementById("check-rules-btn");
...
function selectCharacter(playerId, imgSrc) {
    currentCharacterId = playerId;
    selectedPlayerImage = imgSrc;
    document.getElementById("ally-image").src = selectedPlayerImage; 
    
    // キャラ選択時に「upgradeCritMult」も初期化
    if (playerId === 1) {
        playerStats = { maxHp: 1500, baseAtk: 15, critRate: 5, critDmg: 50, upgradeCritMult: 2 };
    } else {
        playerStats = { maxHp: 800, baseAtk: 10, critRate: 25, critDmg: 80, upgradeCritMult: 2 };
    }
...
// ▼ ハンバーガー・ポーズメニュー機能 ▼
menuBtn.onclick = () => {
    if (!isGameActive) return; // 攻撃中やセリフ中は無効
    isGameActive = false; // ゲームの進行を一時停止
    pauseMenu.style.display = "flex";
};

resumeBtn.onclick = () => {
    pauseMenu.style.display = "none";
    isGameActive = true; // ゲーム再開
};

checkRulesBtn.onclick = () => {
    pauseMenu.style.display = "none";
    rulesPanel.classList.remove("hide-to-menu");
    
    // 現在のダメージ表を生成
    const list = document.getElementById("rules-damage-list");
    list.innerHTML = "";
...
closeRulesBtn.onclick = () => {
    rulesPanel.classList.add("hide-to-menu");
    pauseMenu.style.display = "flex"; // ルールを閉じたらポーズメニューに戻る
};
...
function startEnemyAttack() {
    ...
    enemyAttackInterval = setInterval(() => {
        // ▼ isGameActiveがfalse（ポーズ中など）の場合は敵の攻撃が止まる
...
shootBtn.onclick = async () => {
    ...
    if (buffTriggered) {
        ...
        el2.className = "crit-popup";
        el2.innerText = "KNEE BUFF!\nNEXT +2 ATKS!";
        el2.style.top = "40%";
...
function updateUpgradeUI() {
    ...
    document.getElementById("stat-upg-mult-val").innerText = "x" + playerStats.upgradeCritMult;
...
document.querySelectorAll(".upgrade-btn").forEach(btn => {
    btn.onclick = (e) => {
        ...
        // ★ 10%の確率で強化大成功（クリティカル強化）
        let isCritUpgrade = Math.random() < 0.1;
        
        // 大成功なら「upgradeCritMult」の倍率がかかる（通常は1倍）
        let multiplier = isCritUpgrade ? playerStats.upgradeCritMult : 1;
        
        if (isCritUpgrade) {
            // 大成功の超ド派手ポップアップ演出！
            const popup = document.createElement("div");
            popup.innerText = `UPGRADE CRITICAL!! x${multiplier}`;
            popup.style.position = "absolute";
            popup.style.top = "50%";
            popup.style.left = "50%";
            popup.style.transform = "translate(-50%, -50%)";
            popup.style.color = "var(--neon-pink)";
            popup.style.textShadow = "0 0 20px var(--neon-pink), 0 0 40px #fff";
        ...
        // 上昇量 × 大成功時の倍率
        if(stat === "hp") playerStats.maxHp += 1000 * multiplier;
        else if(stat === "atk") playerStats.baseAtk += 20 * multiplier;
        else if(stat === "crate") playerStats.critRate += 10 * multiplier; // 指定通り +10%
        else if(stat === "cdmg") playerStats.critDmg += 60 * multiplier;   // 指定通り +60%
        else if(stat === "upg_mult") playerStats.upgradeCritMult += 1 * multiplier;  // 大成功倍率自体も強化可能
